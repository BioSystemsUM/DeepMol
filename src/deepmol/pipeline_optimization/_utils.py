...
from deepmol.pipeline_optimization._deepchem_models_objectives import gat_model_steps, gcn_model_steps, \
    ...
    weave_model_steps, dmpnn_model_steps
...
def classification_objective(trial,
    ...
    #     cnn_kwargs = {'n_tasks': n_tasks, 'mode': 'classification', 'n_features': n_features, 'dims': dims}
    ...
    #     featurizer = _get_featurizer(trial, '1D')
    #     n_features = len(featurizer.feature_names)
    #     progressive_multitask_classifier_kwargs = {'n_tasks': n_tasks, 'n_features': n_features}
    ...
    #     featurizer = _get_featurizer(trial, '1D')
    #     n_features = len(featurizer.feature_names)
    #     robust_multitask_classifier_kwargs = {'n_tasks': n_tasks, 'n_features': n_features}
    ...
    #     final_steps.extend([featurizer, model_step[0])
        ...
        chem_ception_kwargs = {'n_tasks': n_tasks}
        ...
        smiles_to_vec_kwargs = {'n_tasks': n_tasks, 'mode': 'classification'}
        ...
        text_cnn_kwargs = {'n_tasks': n_tasks, 'mode': 'classification'}
        ...
        weave_kwargs = {'n_tasks': n_tasks, 'mode': 'classification'}
        steps_weave = weave_model_steps(trial=trial, weave_kwargs=weave_kwargs)
        final_steps.extend(steps_weave)
    elif steps == "dmpnn_model":
        # TODO: add support for different n_classes
        dmpnn_kwargs = {'n_tasks': n_tasks, 'mode': 'classification', 'n_classes': 2}
...
def regression_objective(trial, data: Dataset):
    pass
