...
def rf_model_builder(n_estimators=100, max_features='auto', class_weight={0: 1., 1: 1.}):
...
def create_dense_model(input_dim=1024,
    ...
    #assert n_hidden_layers+2 = len(layers_units) == len(activations) == len(dropouts)+1 == len(batch_normalization)+1 len(l1_l2)+2
    # create model
    model = Sequential()
    ...
    for i in range(n_hidden_layers):
        model.add(Dense(layers_units[i+1], activation=activations[i+1], kernel_regularizer=regularizers.l1_l2(l1=l1_l2[i+1][0], l2=l1_l2[i+1][1])))
