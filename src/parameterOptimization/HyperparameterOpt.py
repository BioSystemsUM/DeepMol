...
class GridHyperparamOpt(HyperparamOpt):
  ...
  def hyperparam_search(self,
    ...
    for ind, hyperparameter_tuple in enumerate(itertools.product(*hyperparam_vals)):
      ...
      if (use_max and valid_score >= best_validation_score) or (not use_max and valid_score <= best_validation_score):
      ...
      print("Model %d/%d, Metric %s, Validation set %s: %f" %(ind + 1, number_combinations, metric.name, ind, valid_score))
