...
class TestMixedFeaturizer(FeaturizerTestCase, TestCase):
    def test_featurize(self):
        ...
        MixedFeaturizer(featurizers=descriptors).featurize(self.mock_dataset, inplace=True)
    ...
    def test_featurize_with_nan(self):
        ...
        descriptors = [All3DDescriptors(mandatory_generation_of_conformers=True), MorganFingerprint()]
        MixedFeaturizer(featurizers=descriptors).featurize(dataset, inplace=True)
    ...
    def test_mixed_featurizer(self):
        ...
        featurize_method = MixedFeaturizer(featurizers=[morgan_fingerprint, atom_pair])
        featurize_method.featurize(self.mock_dataset, inplace=True)
