...
class TestSklearnHyperparameterOptimization(ModelsTestCase, TestCase):
    ...
    def test_fit_predict_evaluate2(self):
        dataset = CSVLoader("CHEMBL217_reduced.csv", id_field="Original_Entry_ID",
                            smiles_field="SMILES", labels_fields=["Activity_Flag"]).create_dataset()
        MorganFingerprint().featurize(dataset)
        train_dataset, valid_dataset, test_dataset = RandomSplitter().train_valid_test_split(dataset, frac_train=0.8,
                                                                                             frac_valid=0.1,
                                                                                             frac_test=0.1)
